...
if num=="2":
	for i in xrange(0,len(filelist)-1,2):
		...
		p1= subprocess.Popen("bbmap.sh ref=%s ambig=all nzo=t mappedonly=t scafstats=%s%s.map in1=%s in2=%s outu=%s%s.unmapped.fastq overwrite=t" %(ref,mappedfolder,name1,filelist[i],filelist[i+1],unmappedfolder,name1), shell=True).wait() 
